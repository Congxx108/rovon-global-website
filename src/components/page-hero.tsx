    ...
    <>
      ...
      <section className="overflow-hidden border-b border-[#d8d8d8] bg-white text-graphite-950">
        ...
        <div className="container-shell py-12 text-center md:py-16">
          <nav className="hero-kicker mb-3 text-[10px] font-bold uppercase tracking-[0.18em] text-graphite-500" aria-label="Breadcrumb">
            Home / {title}
          </nav>
          <h1 className="hero-title mx-auto max-w-4xl text-4xl font-black leading-[1.05] text-graphite-950 md:text-6xl">{title}</h1>
          <p className="hero-subtext mx-auto mt-5 max-w-3xl text-sm leading-7 text-graphite-600 md:text-base md:leading-8">
            {description}
          </p>
          {children ? <div className="hero-actions mt-6 flex flex-wrap justify-center gap-3">{children}</div> : null}
        </div>
      </section>
    </>
  );
