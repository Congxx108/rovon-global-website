...
export function ProductCard({ category, featured = false }: ProductCardProps) {
  ...
  return (
    <Link
      ...
      className="focus-ring interactive-card group flex h-full flex-col overflow-hidden rounded-md border border-stonebrand-200 bg-white shadow-[0_6px_18px_rgba(0,0,0,0.05)] hover:border-graphite-300"
    >
      <div
        className={`interactive-media relative overflow-hidden bg-[#f6f6f6] bg-[url('/images/generated/luggage-product-catalog.png')] bg-[length:210%_210%] bg-no-repeat ${featured ? "aspect-[16/9] lg:aspect-[16/10]" : "aspect-[4/3]"}`}
